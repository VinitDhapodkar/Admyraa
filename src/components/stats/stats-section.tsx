        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {SITE_DATA.stats.map((stat, i) => (
            <ScrollReveal
              key={stat.label}
              delay={i * 0.1}
              className="p-6 sm:p-7 lg:p-8 rounded-3xl bg-white/90 border-2 border-slate-200/80 hover:bg-white hover:border-amber-400 hover:shadow-xl hover:shadow-amber-950/5 hover:-translate-y-1.5 transition-all duration-300 text-center group flex flex-col justify-between shadow-sm"
            >
              <div>
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-amber-50 text-amber-700 border border-amber-200/60 flex items-center justify-center mx-auto mb-4 sm:mb-5 group-hover:scale-110 group-hover:bg-[#0b1f3a] group-hover:text-amber-400 group-hover:border-[#0b1f3a] transition-all shadow-sm">
                  {iconMap[stat.icon]}
                </div>
              ...
              <p className="text-xs sm:text-sm font-bold text-slate-600 mt-2">{stat.label}</p>
            </ScrollReveal>
          ))}
        </div>

        {/* Social Proof Trust Pill (matching reference's "Loved by thousands..." banner) */}
        <ScrollReveal delay={0.4} className="mt-10 sm:mt-12 text-center px-2">
          <div className="inline-flex flex-wrap items-center justify-center gap-2 sm:gap-3 px-4 py-2.5 sm:px-6 sm:py-3 rounded-2xl sm:rounded-full bg-slate-900 text-white border border-amber-400/30 shadow-lg text-xs sm:text-sm font-semibold max-w-full text-center">
